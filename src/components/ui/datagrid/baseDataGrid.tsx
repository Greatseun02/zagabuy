...
import StatusIndicator, {StatusIndicatorProps,} from "../../custom/statusIndicator";
import StatusRenderer from "./renderers/statusRenderer";
import {AgGridReact} from "ag-grid-react";
import BaseTableHeaderControls, {HeaderControlsProps,} from "./tablePanels/baseTableHeaderControls";
import CustomTableEmptyState from "./components/customTableEmptyState";

import * as XLSX from "xlsx-js-style";
import {saveAs} from "file-saver";
import {EditActionRenderer, EditActionRendererProps,} from "@/components/ui/datagrid/renderers/editActionRenderer";
import {
    DeleteActionRenderer,
    DeleteActionRendererProps,
} from "@/components/ui/datagrid/renderers/deleteActionRenderer";
import {ViewActionRenderer, ViewActionRendererProps,} from "@/components/ui/datagrid/renderers/viewActionRenderer";
...
> = {
    ...
    /** Misc */
    customization?: DataGridCustomization; showSerialNumberColumn?: boolean;
    suppressDefaultStatusRenderer?: boolean;
    expandCols?: boolean;

    /** Row click - callback for custom actions (opens modal, shows detail, etc.) */
    onRowClick?: (data: TData, event: RowClickedEvent<TData>) => void | Promise<void>;
...
const BaseDataGrid = memo(
    forwardRef<BaseDataGridRef, BaseDataGridProps<Record<string, unknown>>>(
        (props, ref) => {
                ...
                async (
                    ...
                    try {
                        ...
                        if (paginationMode === "server" && props.fetchRows) {
                            ...
                            const response = await finalOperationConfig.fetchRows(
                                serverParams
                            );
                            ...
                            let totalRecords = response?.totalRecords ?? (rawResponse?.totalItems as number | undefined);
            ...
            const filterActions = useMemo(
                () => ({
                    ...
                    applyFilters: () => {
                        ...
                        const mergedFilters = {...filterState.columnFilters, ...flatDateFilters};
            ...
            const addSerialNumberColumn = useCallback((data: Record<string, unknown>[]): Record<string, unknown>[] => {
            ...
            const applyExcelStyling = useCallback((worksheet: XLSX.WorkSheet): void => {
            ...
            const handleExportData = useCallback(async (format: "csv" | "excel") => {
                ...
                try {
                    ...
                    if (paginationMode === "server" && props.fetchRows && pagination.totalRecords > 0) {
                        ...
                        dataToExport = (response?.data || []) as Record<string, unknown>[];
                    } else {
                        dataToExport = (paginationMode === "server" ? rowData : (props.rows || [])) as Record<string, unknown>[];
                    }

                    const datestamp = new Date().toISOString().slice(0, 10);
                    const baseName = title ? title.replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_-]/g, "") : "export";
                ...
                paginationMode, pagination.totalRecords, props.fetchRows, props.maxExportRows,
                rowData, props.rows, title, isServerPageZeroIndexed,
            ...
            const enhancedGridOptions: GridOptions<Record<string, unknown>> =
                useMemo(() => {
                    const baseGridOptions: GridOptions<Record<string, unknown>> = {
                        ...
                        rowSelection: props?.defaultHeaderProps?.onSelectRows ? "multiple" : undefined,
            ...
            const selectionColumnDef: SelectionColumnDef = useMemo(() => {
                return {
                    ...
                    headerClass: "bg-muted font-medium text-xs text-muted-foreground border-b-0",
                };
            }, []);
            ...
            const hasActiveFilters = useMemo(() => {
                return Boolean(
                    Object.keys(filterState.columnFilters).some(
                        key => filterState.columnFilters[key] && filterState.columnFilters[key] !== ""
                    ) || filterState.searchText || Object.keys(filterState.dateFilters).some(
                        key => filterState.dateFilters[key]?.from || filterState.dateFilters[key]?.to
                    )
                );
            }, [filterState]);
