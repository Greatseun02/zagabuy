export class StringUtil {
  ...
  static abbreviateToTwoCharacter(word: string): string {
    ...
    const formattedWord =
      trimmedWord.slice(0, trimmedWord.indexOf(" ")) +
      trimmedWord.slice(trimmedWord.lastIndexOf(" "));
    ...
    if (formattedWord.length > 1) {
      if (formattedWord.split(" ").length > 1)
        return `${formattedWord.split(" ")[0][0]}${
          formattedWord.split(" ")[formattedWord.split(" ").length - 1][0]
        }`.toUpperCase();
